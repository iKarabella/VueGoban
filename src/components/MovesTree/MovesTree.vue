...
const branches = ref([]);
const branchLines = ['#ff0000', '#0f000e', '#e2a344', '#a5ef6f', '#cdfefe'];
const branchIndent = ref(0);

const readBranch = function(branch, nodes=[], addNode=null)
{
    let currentBranch = {
        moves:[],
    ...
    for(let i=branch.length-1; i>=0; i--)
    {
        ...
        {
            branchIndent.value++;
            branches.push(...readBranch(branch[i].children, nodes.slice(0), branch[i].number));
        }
    }
        ...
        <svg :width="svgWidth">
            ...
            <g v-for="(branch, bi) in branches" :key="bi" :transform="`translate(${(elSize+5)*((branch.moves[0].number??1)-1)},0)`">
                <path v-if="branch.indent>0" 
                    :d="calculatePath({x:-(elSize/2+5), y:elSize+(elSize+5)*(bi-branch.indent)}, {x:0, y:elSize/2+(elSize+5)*bi})"
                    :stroke="branchLines[bi]" 
                    stroke-width="1"
                    fill="none">
                </path>
