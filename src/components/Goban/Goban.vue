...
const hoverVertice = function(coords, hover=true)
{
    ...
    else if (props.game.currentMode=='black') {
        input = {
            href:'#black-1',
            opacity:0.6
        };
    ...
    if(!vertice) 
    {
        vertice = {
            x:coords[0], 
            y:coords[1],
            transform:`translate(${(fieldSize.value-1+(elSize.value+1)*(coords[1]-1))}, ${(fieldSize.value-1+(elSize.value+1)*(19-coords[0]))})`,
            contains:[]
        };
...
<template>
    <div class="goban-container">
        <svg :width="`${svgSize.width}px`" :height="`${svgSize.height}px`" style="border:1px solid gray">
            ...
            <g>
                <g v-for="(v,index) in grid" :key="index" :transform="v.transform">
                    <use v-for="e in v.contains" :href="e.href" x="-0.5" y="-0.5" :opacity="e.opacity"></use>
                </g>
            </g>
        </svg>
